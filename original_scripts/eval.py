    ...
    def run_eval(self):
        ...
        if self.split == "val":
            masks_sep_dir = os.path.join(
                "../models", args.model_name, "masks_" + cfg.PATH.SEQUENCES.split("/")[-3]
            )
            # masks_sep_dir = os.path.join('../models', args.model_name, 'masks_sep_2assess-davis')
            make_dir(masks_sep_dir)
            if args.overlay_masks:
                results_dir = os.path.join(
                    "../models", args.model_name, "results-" + cfg.PATH.SEQUENCES.split("/")[-3]
                )
            ...
            for batch_idx, (inputs, targets, seq_name, starting_frame, imgs_ela) in enumerate(self.loader):
                ...
                for ii in range(max_ii):
                    ...
                    for t in range(1):
                        ...
                        np.save(
                            base_dir_masks_sep + "%05d_rawscore_%02d.npy" % (starting_frame[0] + ii, t),
                            mask_pred,
                        )
                    if args.overlay_masks:
                ...
                for ii in range(max_ii):
                    ...
                    if ii == 0:
                        annotation = Image.open(
                            "../databases/DAVIS2016/Annotations/480p/" + seq_name[0] + "/00000.png"
                        )
                    ...
                    outs, hidden_temporal_list = test_ela(
                        args, self.encoder, self.decoder, x, prev_hidden_temporal_list
                    )
