...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_, prev_state_spatial, hidden_state_temporal):
        ...
        if prev_state_spatial is None:
            ...
            prev_state_spatial = (
                Variable(torch.zeros(state_size)).to(input_.device),
                Variable(torch.zeros(state_size)).to(input_.device),
            )
            # if self.use_gpu:
            #     prev_state_spatial = (
        ...
        if hidden_state_temporal is None:
            state_size = [batch_size, self.hidden_size] + list(spatial_size)
            hidden_state_temporal = Variable(torch.zeros(state_size)).to(input_.device)
            # if self.use_gpu:
...
class ConvLSTMCellMask(nn.Module):
    ...
    def __init__(self, args, input_size, hidden_size, kernel_size, padding):
        ...
        self.Gates = nn.Conv2d(
            input_size + 2 * hidden_size + 1, 4 * hidden_size, kernel_size, padding=padding
        )
