...
class noise_filter(nn.Module):
    def __init__(
        ...
        self.weight = Variable(torch.from_numpy(Wcnn), requires_grad=True).to(self.device)
...
def to_tridiagonal_multidim(w):
    ...
    eye_a = Variable(torch.diag(torch.ones(W - 1), diagonal=-1)).to(w.device)
    eye_b = Variable(torch.diag(torch.ones(W), diagonal=0)).to(w.device)
    eye_c = Variable(torch.diag(torch.ones(W - 1), diagonal=1)).to(w.device)
...
def recursive_filter(feature, tmp, direction):
    ...
    rnn_h1 = Variable(torch.zeros((N, C, H, W))).to(feature.device)
