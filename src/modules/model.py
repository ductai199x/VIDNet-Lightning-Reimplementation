...
class FeatureExtractor_segment(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.sk3 = nn.Conv2d(
            skip_dims_in[2], int(self.hidden_size / 2), self.kernel_size, padding=self.padding
        )
        self.sk2 = nn.Conv2d(
            skip_dims_in[3], int(self.hidden_size / 4), self.kernel_size, padding=self.padding
        )
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.sk5 = nn.Conv2d(skip_dims_in[0], int(self.hidden_size), self.kernel_size, padding=self.padding)
        self.sk4 = nn.Conv2d(
            skip_dims_in[1] * 2, int(self.hidden_size), self.kernel_size, padding=self.padding
        )
        self.sk3 = nn.Conv2d(
            skip_dims_in[2] * 2, int(self.hidden_size / 2), self.kernel_size, padding=self.padding
        )
        self.sk2 = nn.Conv2d(
            skip_dims_in[3] * 2, int(self.hidden_size / 4), self.kernel_size, padding=self.padding
        )
        self.sk1 = nn.Conv2d(
            skip_dims_in[4] * 2, int(self.hidden_size / 8), self.kernel_size, padding=self.padding
        )
...
class RSIS(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(len(skip_dims_out) + 1):
            if i == 0:
                self.aspp_f.append(
                    nn.Conv2d(skip_dims_out[0], skip_dims_out[0], self.kernel_size, padding=padding)
                )

            else:
                self.aspp_f.append(
                    nn.Conv2d(skip_dims_out[i - 1], skip_dims_out[i - 1], self.kernel_size, padding=padding)
                )
