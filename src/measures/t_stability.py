...
def sc_compute(Bsamp, Tsamp, mean_dist, nbins_theta, nbins_r, r_inner, r_outer, out_vec):
    ...
    theta_array_abs0 = Bsamp[1, :].reshape(-1, 1).dot(np.ones((1, nsamp))) - np.ones((nsamp, 1)).dot(
        Bsamp[1, :].reshape(1, -1)
    )

    theta_array_abs1 = Bsamp[0, :].reshape(-1, 1).dot(np.ones((1, nsamp))) - np.ones((nsamp, 1)).dot(
        Bsamp[0, :].reshape(1, -1)
    )
    ...
    for n in range(nsamp):
        ...
        coords = np.hstack(
            (theta_array_q[n, fzn].reshape(-1, 1), r_array_q[n, fzn].astype(np.int).reshape(-1, 1))
        )
...
def db_eval_t_stab(fgmask, ground_truth, timing=True):
    ...
    if len(poly1.contour_coords) == 0 or len(poly2.contour_coords) == 0:
        return np.nan
