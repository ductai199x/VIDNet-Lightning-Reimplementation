    ...
    for obj_id in annotations.iter_objects_id():
        results["raw"].append(
            Parallel(n_jobs=n_jobs)(
                delayed(_db_measures[measure])(an == obj_id, sg == obj_id)
                for an, sg in zip(annotations[1:-1], segmentations[1:-1])
            )
        )
...
def db_eval(db, segmentations, measures, n_jobs=cfg.N_JOBS, verbose=True):
    ...
    for measure in measures:
        ...
        for statistic in cfg.EVAL.STATISTICS:
            raw_data = np.hstack([s_eval[sequence][measure][statistic] for sequence in s_eval.keys()])
...
def print_results(evaluation, method_name="-"):
    ...
    # Print results
    table = PrettyTable(
        ["Method"] + [p[0] + "_" + p[1] for p in itertools.product(metrics, cfg.EVAL.STATISTICS)]
    )

    table.add_row(
        [method_name]
        + [
            "%.3f" % np.round(evaluation["dataset"][metric][statistic], 3)
            for metric, statistic in itertools.product(metrics, cfg.EVAL.STATISTICS)
        ]
    )
